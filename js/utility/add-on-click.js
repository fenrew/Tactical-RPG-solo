...
const addPlayerPanelInfoText = (player) => {
    ...
    newHeaderEle.classList.add(`player${player.playerNumber}-panel-char-info-header`)
    newTextEle.id = "panel-char-info-text-box"

    newHeaderEle.innerText = `Player: ${player.playerNumber+1} (${player.class.className})`
    
    const {hp, currentHp, mana, currentMana, currentMovementPoints, maxMovementPoints} = player.class.combatstats
    ...
    newMpEle.style.color = `rgb(${(currentMovementPoints*135/maxMovementPoints)+120}, 0, 0)`

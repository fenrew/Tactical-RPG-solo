...
function findAvailableMovementArea(player, map, movementPoints) {
  let newMovementMap,
  ...
  newMovementMap[player.position.y][
    player.position.x
    ...
    for (let y = 0; y < newMovementMap.length; y++) {
      for (let x = 0; x < newMovementMap[y].length; x++) {
        if (newMovementMap[y][x] === movement + 1) {
          if (
            checkIfInsideOfMap(newMovementMap, y - 1, x) &&
          ...
          if (
            checkIfInsideOfMap(newMovementMap, y, x + 1) &&
...
function confirmMovementToPosition(position, availableMovementMap) {
  ...
  Game.activeMap[player.position.y][player.position.x] =
    Game.originalMap[player.position.y][player.position.x];
  if (player.npc) {
    player.class.combatstats.currentMovementPoints =
      availableMovementMap[position.y][position.x] - 1;
    //BUG: NPC 7,15. cannot read property "y" of undefined
  } else {
    player.class.combatstats.currentMovementPoints =
      Game.availableMovementMap[position.y][position.x] - 1;
  }

...
class AggressiveAi {
  ...
  setActiveMap() {
    ...
    this.spellRangeMap = this.activeMap.map((y) => {
      return y.map(() => {
        return [];
      });
    });
    this.damageMap = this.activeMap.map((y) => {
      return y.map(() => {
        return 0;
      });
    });
  ...
  fillDamageMap() {
    Game.combatTimeline.forEach((ele) => {
      if (ele.npc) return;
      let allSpellRangeMap = this.activeMap.map((y) => {
        return y.map(() => {
          return 0;
        });
      });
      ...
      for (let spell in this.spellList) {
        let spellRangeMap = calculateSpellRangeFreeCells(
          allSpellRangeMap,
          this.spellList[spell].spellInfo,
          ele.position
        );
        spellDamageOnPlayerObject.spell.push({
          spell: JSON.parse(JSON.stringify(this.spellList[spell])),
          spellRangeMap,
        });
      }
  ...
  calculateAttackWeight() {
    this.spellDamageOnPlayer.forEach((player) => {
      ...
      player.spell.sort((a, b) => {
        return a.spell.spellInfo.canBeCast
          ? b.spell.spellInfo.aiWeight - a.spell.spellInfo.aiWeight
          : a.spell.spellInfo.aiWeight - b.spell.spellInfo.aiWeight;
      });

      for (let i = 0; i < player.spell.length; i++) {
        if (
          !JSON.stringify(player.spell[i].spellRangeMap).includes(
            player.spell[i].spell.spellInfo.damage.toString()
          ) ||
      ...
      while (currentMana > 0 && whileLoopContinue) {
        let remainingSpells = player.spell.filter(
          (ele) =>
            ele.spell.spellInfo.manaCost <= currentMana &&
            ele.spell.spellInfo.canBeCast
        );
  ...
  findMovementPathForAttacking(player) {
      ...
      for (let x = 0; x < spellRangeMap[y].length; x++) {
        if (spellRangeMap[y][x] > 0 && this.movementMap[y][x] > 0) {
          if (bestMovementPosition.remainingMovement < this.movementMap[y][x]) {
            bestMovementPosition = {
              position: { x, y },
              remainingMovement: this.movementMap[y][x],
            };
          }
        }
      }
    ...
    player.bestMovementRoute = findBestMovementRoute(
      bestMovementPosition.position,
      this.movementMap
    );
    ...
    player.castSequence.forEach((spell, index) => {
      ...
      for (let y = 0; y < player.bestMovementRoute.length; y++) {
        for (let x = 0; x < player.bestMovementRoute[y].length; x++) {
          if (
            player.bestMovementRoute[y][x] == "path" &&
            spell.spellRangeMap[y][x] > 0
          ) {
            castPosition = { y, x };
          }
  ...
  executeMovementAndActions() {
    ...
    if (this.spellDamageOnPlayer[0].bestMovementPosition) {
      confirmMovementToPosition(
        this.spellDamageOnPlayer[0].bestMovementPosition,
        this.movementMap
      ); // BUG: NPC position: 10,12, cannot read property y of undefined (movement.js:47)
      this.executeMovement(
        JSON.parse(
          JSON.stringify(this.spellDamageOnPlayer[0].bestMovementRoute)
        ),
        originalPosition
      );
    }
  ...
  findBestMovementRoute(
    originalStartPosition,
    destinationPosition,
    originalAvailableMovementMap
  ) {
    let availableMovementMap = JSON.parse(
      JSON.stringify(originalAvailableMovementMap)
    );
    let yPos = originalStartPosition.y,
      xPos = originalStartPosition.x,
      currentNumber = availableMovementMap[yPos][xPos],
    ...
    while (!originalPosition) {
      ...
      if (
        xPos + 1 < availableMovementMap[yPos].length &&
        ...
        yPos + 1 < availableMovementMap.length &&
  ...
  executeMovement(movementRoute, position) {
    ...
    let spellsToCast = this.spellDamageOnPlayer[0].castSequence.filter(
      (spell) => {
        return (
          spell.castPosition.y == position.y &&
          spell.castPosition.x == position.x
        );
      }
    );
    ...
    if (movementRoute[y][x + 1] == "path") {
      ...
      y + 1 < movementRoute.length &&
      movementRoute[y + 1][x] == "path"
    ) {
      movementRoute[y + 1][x] = 0;
      y += 1;
      return this.yMovementVisuals(
        y - 1,
        y,
        x,
        playerElement,
  ...
  clearMovementVisuals(classElement, blockElement, x, y) {
    ...
    document
      .getElementById("map-grid-block-" + y + "," + x)
      .appendChild(classElement);
  }
      ...
      for (let y = 0; y < movementMap.length; y++) {
        for (let x = 0; x < movementMap[y].length; x++) {
          if (movementMap[y][x] == counter) {
            if (
              checkIfInsideOfMap(movementMap, y - 1, x) &&
              movementMap[y - 1][x] === 0
            ) {
            ...
            if (
              checkIfInsideOfMap(movementMap, y, x + 1) &&
              movementMap[y][x + 1] === 0
            ) {
  ...
  executeNoSpellMovement() {
    ...
    let destinationPosition,
      iterationPosition = { y: this.npc.position.y, x: this.npc.position.x };
    let currentNumber = this.movementWeight[0].movementMap[this.npc.position.y][
      this.npc.position.x
    ];
    ...
    if (currentNumber > remainingMovement) {
      ...
      while (!destinationPosition) {
        if (
          this.movementWeight[0].movementMap[iterationPosition.y][
            iterationPosition.x
          ] == remainingMovement ||
          currentNumber == 2
        ) {
        ...
        if (
          iterationPosition.x + 1 <
            this.movementWeight[0].movementMap[iterationPosition.y].length &&
          this.movementWeight[0].movementMap[iterationPosition.y][
            iterationPosition.x + 1
          ] < currentNumber &&
          this.movementWeight[0].movementMap[iterationPosition.y][
          ...
          iterationPosition.y + 1 < this.movementWeight[0].movementMap.length &&
          this.movementWeight[0].movementMap[iterationPosition.y + 1][
            iterationPosition.x
          ] < currentNumber &&
          this.movementWeight[0].movementMap[iterationPosition.y + 1][
            iterationPosition.x
    ...
    let bestMovementRoute = this.findBestMovementRoute(
      this.npc.position,
      destinationPosition,
      this.movementWeight[0].movementMap
    );
